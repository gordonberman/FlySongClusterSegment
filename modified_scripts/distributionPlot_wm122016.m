function handles = distributionPlot_wm122016(varargin)
%%%Modified WKM 12/20/16: Renamed subroutine histogram.m ->
%%%histogram_distPlot.m, in order to avoid over-writing MATLAB built-in
%%%function 'histogram'
...
%%% Modified WKM 12/20/16:
%                   1.1: smoothened histogram using ksdensity where the
%                       kernel is robustly estimated via histogram_distPlot.m.
...
% SEE ALSO histogram_distPlot, ksdensity, plotSpread, boxplot, grp2idx
        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    % use histogram_distPlot to estimate kernel
                    %[dummy,x] = histogram(currentData); %#ok<ASGLU>
                    [dummy,x] = histogram_distPlot(currentData); %#ok<ASGLU>
                    if length(x) == 1
            ...
            case 2
                ...
                % use histogram_distPlot - bar heights are counts as in hist
                %[xHist,yHist] = histogram(currentData,opt.divFactor,0);
                [xHist,yHist] = histogram_distPlot(currentData,opt.divFactor,0);
            case 99
